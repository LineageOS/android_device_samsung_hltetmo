...
if [[ $CMDLINE != "${CMDLINE/SM-N900T}" ]]; #hltetmo
    ...
    then
       ...
       sed -i 's|ro.build.description=xxxxxxxxxx|ro.build.description="hltevl-user 4.3 JSS15J N900W8VLUBMJ4 release-keys|g' /tmp/build.prop
    ...
    then
       sed -i 's|ro.build.fingerprint=xxxxxxxxxx|ro.build.fingerprint=Verizon/hltevzw/hltevzw:4.3/JSS15J/N900VOYUBMJ3:user/release-keys|g' /tmp/build.prop
       sed -i 's|ro.build.description=xxxxxxxxxx|ro.build.description=hltevzw-user 4.3 JSS15J N900VOYUBMJ3 release-keys|g' /tmp/build.prop
       sed -i 's|ro.product.model=hlte|ro.product.model=SM-N900W8|g' /tmp/build.prop
        sed -i 's|ro.product.device=hlte|ro.product.device=hltevzw|g' /tmp/build.prop
       sed -i 's|ro.cm.device=hlte|ro.cm.device=hltevzw|g' /tmp/build.prop
